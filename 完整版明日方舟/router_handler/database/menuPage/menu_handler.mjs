...
export function addMedical(req,res){
    ...
    const sql = `insert into users.medical_basic set ?`
...
export function queryMedical(req,res){
    ...
    const sql = `select * from users.medical_basic where id=?`
...
export function setMedical(req,res){
    ...
    const sql = `update users.medical_basic set ? where id = ?`
...
export function delMedical(req,res){
    ...
    console.log(id)
    const sql = `DELETE FROM medical_basic WHERE id= ?`
    mysql.query(sql,id,(err,result)=>{
    console.log(err)
        if(err) return res.cc(err)
