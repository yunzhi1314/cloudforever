<template>
  <div>
    ...
    <div class="login" :style="{
      ...
      <div class="info">
        <section v-for="(item, index) in loginArr" :key="index">
          <p>
            <input :type="item.type" :placeholder="item.placeholder" v-model="item.value" :style="{
              backgroundColor: item.isShow ? '#FFDCDB' : '',
            }" />
            <button v-if="controlObj.isChange
                ? index == 3
                : controlObj.isCode && index == 1
              " class="btn" @click="getMathCode('isMathCode')" :style='{
    backgroundColor: controlObj.isDisable ? "#158FC5" : "#797979"
      ...
      <div>
        <section>
          <input type="checkbox" v-if="controlObj.isChange" style="width: 1vw; height: 2vh" />
          <span @click="codeLogin" :style="{ color: controlObj.isCode ? '#000' : '' }">{{
            controlObj.isChange
            ? "已阅读并同意"
            : controlObj.isCode
              ? "使用密码登录"
...
import { watcher } from "@/hook/personalCenter/watcher"; //监视函数
import controlObj from "@/hook/personalCenter/control";
  ...
  setup() {
    ...
    function loginOrRegister() {
      ...
      if (controlObj.isChange) {
        ...
        Request.post(url.personalCenter.login, loginData).then((res) => {
          dataList.data1 = res.data;
        });
        setTimeout(() => {
          store.commit("personalCenter/changeToken", dataList.data1);
          store.commit("CHANGE_STORE", "isLogin");
          router.push({
            name: "bufferPage",
            params: {
              userId: JSON.parse(localStorage.getItem("users")).userId,
