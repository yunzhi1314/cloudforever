<template>
  <div style="display: flex; justify-content: space-around; align-items: center">
    <div v-for="(item, index) in data.menu" :key="index" class="plans">
      <div>
        <section>
        ...
        <section style="margin-top: 1vh; font-size: 14px">
          <span>{{ item.word }}</span>
        </section>
      </div>
  ...
  <div style="display: flex">
    ...
    >
      ...
      <div class="tools">
        <div v-for="(item,index) in data.menu" :key="index" class="tool">
          <section>
            <span>{{ item.word }}</span>
          </section>
...
<script>
import { homeData } from "@/api/database/home";
...
export default {
  ...
  setup() {
    ...
    return {
      ...
      homePageCSS,
      ...toRefs(homeData(url.database.home.navTools))
    };
  }
};
</script>
