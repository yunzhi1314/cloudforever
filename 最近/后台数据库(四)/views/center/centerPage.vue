    ...
    <header :style='{
        ...
        <div class="right">
            <section class="leftSectionParent" v-if='bool' @click="loginOut">
                <img :src="menuMsg.loginOut" alt="" style="width:2.25rem;margin-left: 0.5vw;">
...
import { toRefs,computed,provide,ref,reactive } from "vue";
...
export default {
    setup() {
        ...
        if(bool.value){
            // 将菜单信息导出到页面中使用
            menuMsg = reactive(JSON.parse(sessionStorage.getItem("menus"))).menuMsg
            pages = reactive(JSON.parse(sessionStorage.getItem("menus"))).routes
