<template>
  <div class="box">
    ...
    <el-form class="box_1" :model="tel" :rules="formRules" ref="ruleFormRef">
      <div class="heact">
        <section class="section">验证码登录</section>
        <section @click="func">密码登录</section>
      </div>
      ...
      >
        <el-input
          style="
            ...
            position: relative;
          "
          :placeholder="index == 1 ? '请输入密码' : item.text"
          v-model="item.value"
        />
        <span
          v-show="index == 1"
    ...
    <TusiVue v-show="offno.tusi" :class="offno.tusi ? 'tusi' : ''"></TusiVue>
  </div>
</template>
...
let yanzheng = () => {
  ...
  if (zz.test(tel.telephone)) {
    //动态管理器vuex
    offno.yanzheng = true
    message('验证码发送失败', '验证码以发送')
  } else {
    message('验证码以发送', '验证码发送失败')
  }
}
...
//点击登录
async function logage(formEl: FormInstance | undefined) {
  //校验表单数据
  if (!formEl) return
  await formEl.validate((valid) => {
    if (valid) {
      //发送表单数据到后台
      message('登录成功', '登录失败', '/api/geshin/user/codeLogin', 'post', {
