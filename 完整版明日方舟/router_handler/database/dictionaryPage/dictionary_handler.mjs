...
export function addMedical(req,res){
    ...
    const sql = `insert into users.nawei_finance2 set ?`
...
export function queryMedical(req,res){
    ...
    const sql = `select * from users.nawei_finance2 where id=?`
...
export function setMedical(req,res){
    ...
    const sql = `update users.nawei_finance2 set ? where id = ?`
...
export function delMedical(req,res){
    ...
    const sql = `DELETE FROM nawei_finance2 WHERE id= ?`
    mysql.query(sql,id,(err,result)=>{
        if(err) return res.cc(err)
