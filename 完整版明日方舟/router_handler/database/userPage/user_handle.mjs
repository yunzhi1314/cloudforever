...
import {processNumber} from "../../../static/dealData.mjs"

export function addMedical(req,res){
    let userinfo = req.body
    userinfo = processNumber(userinfo)
    console.log(userinfo)

    const sql = `insert into users.medical_experiment_basic set ?`
...
export function queryMedical(req,res){
    ...
    const sql = `select * from users.medical_experiment_basic where id=?`
...
export function setMedical(req,res){
    ...
    const sql = `update users.medical_experiment_basic set ? where id = ?`
...
export function delMedical(req,res){
    ...
    const sql = `DELETE FROM medical_experiment_basic WHERE id= ?`
    mysql.query(sql,id,(err,result)=>{
        if(err) return res.cc(err)
